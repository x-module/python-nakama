...
class NoticeHandler:
    def __init__(self):
        ...
        self._handler:NoticeHandlerInter = None

    def set_handler(self, handler: NoticeHandlerInter):
        self._handler = handler

    def handle_event(self, msg_type: str, event: Envelope):
        if self._handler is not None:
            if msg_type == "channel":
                ...
                self._handler.status_follow(event.status_follow)
            elif msg_type == "status_presence_event":
                self._handler.status_presence_event(event.status_presence_event)
            elif msg_type == "status_unfollow":
                self._handler.status_unfollow(event.status_unfollow)
            elif msg_type == "status_update":
                self._handler.status_update(event.status_update)
            elif msg_type == "stream_data":
                self._handler.stream_data(event.stream_data)
            elif msg_type == "stream_presence_event":
                self._handler.stream_presence_event(event.stream_presence_event)
            elif msg_type == "ping":
                ...
                self._handler.party_create(event.party_create)
            elif msg_type == "party_join":
                self._handler.party_join(event.party_join)
            elif msg_type == "party_leave":
                self._handler.party_leave(event.party_leave)
            elif msg_type == "party_promote":
                self._handler.party_promote(event.party_promote)
            elif msg_type == "party_leader":
                self._handler.party_leader(event.party_leader)
            elif msg_type == "party_accept":
                self._handler.party_accept(event.party_accept)
            elif msg_type == "party_remove":
                self._handler.party_remove(event.party_remove)
            elif msg_type == "party_close":
                self._handler.party_close(event.party_close)
            elif msg_type == "party_join_request_list":
                self._handler.party_join_request_list(event.party_join_request_list)
            elif msg_type == "party_join_request":
                self._handler.party_join_request(event.party_join_request)
            elif msg_type == "party_matchmaker_add":
                self._handler.party_matchmaker_add(event.party_matchmaker_add)
            elif msg_type == "party_matchmaker_remove":
                self._handler.party_matchmaker_remove(event.party_matchmaker_remove)
            elif msg_type == "party_matchmaker_ticket":
                self._handler.party_matchmaker_ticket(event.party_matchmaker_ticket)
            elif msg_type == "party_data":
                self._handler.party_data(event.party_data)
            elif msg_type == "party_data_send":
                self._handler.party_data_send(event.party_data_send)
            elif msg_type == "party_presence_event":
                self._handler.party_presence_event(event.party_presence_event)
            else:
